...
The original author has stopped maintaining it, but DeepL supports new languages, so it was rewritten to keep the feature set current and compatible with new APIs.

## Credits
Thanks to the original projects:
- [clubxdev/bob-plugin-deeplx](https://github.com/clubxdev/bob-plugin-deeplx)
- [OwO-Network/DeepLX](https://github.com/OwO-Network/DeepLX)
- [missuo/bob-plugin-deeplx](https://github.com/missuo/bob-plugin-deeplx)
...
### Linux.do（api.deeplx.org）
如果你已获取 Linux.do 的 API Key，请直接配置插件为：
```
https://api.deeplx.org/你的 API Key/translate
```
并在「Token」中填入同一把 Key。插件会自动将 Key 注入到 URL，自动兼容本地部署接口。

### Download Plugin
[Click here](https://github.com/Sarmay/bob-plugin-deeplx/releases)
...
## Author
**bob-plugin-deeplx** maintained by [Sarmay](https://github.com/Sarmay), with thanks to the original author [Vincent Young](https://github.com/missuo). Released under the [MIT](./LICENSE) License.<br>
